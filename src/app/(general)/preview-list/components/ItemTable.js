...
import { Pencil, Trash2, AlertCircle, File, Check, X, ExternalLink, PlaySquare } from "lucide-react";
                ...
                <td className="py-3">
                  {item.previewUrl ? (
                    <a
                      ...
                      style={{ color: "#6b46c1", fontWeight: "500", fontSize: "0.85rem", cursor: "pointer" }}
                    >
          ...
          <div className="modal-dialog modal-dialog-centered modal-lg">
            <div className="modal-content overflow-hidden border-0 shadow">
              <div className="modal-header border-bottom-0 bg-light">
                <h6 className="modal-title fw-bold text-dark m-0">Media Preview</h6>
                <button type="button" className="btn-close" onClick={() => setPreviewMedia(null)}></button>
              </div>
              <div className="modal-body text-center p-0 bg-dark d-flex align-items-center justify-content-center" style={{ minHeight: '300px' }}>
                {previewMedia.type === 'image' && (
                  <img src={previewMedia.url} className="img-fluid" alt="Preview" style={{ maxHeight: '70vh', objectFit: 'contain' }} />
                )}
                {previewMedia.type === 'video' && (
                  <video src={previewMedia.url} controls autoPlay className="w-100" style={{ maxHeight: '70vh' }}>
                    Your browser does not support the video tag.
                  </video>
                )}
                {previewMedia.type === 'ctv' && (
                  <div className="w-100 bg-white d-flex flex-column">
                    <div className="d-flex justify-content-between align-items-center p-2 border-bottom" style={{ backgroundColor: '#f8fafc' }}>
                      <span className="small text-muted fw-bold ps-2">CTV Preview</span>
                      <a href={getSafeIframeUrl(previewMedia.url)} target="_blank" rel="noreferrer" className="btn btn-sm btn-primary py-1 px-2 d-flex align-items-center" style={{ fontSize: '0.75rem', backgroundColor: '#6b46c1', border: 'none' }}>
                        <ExternalLink size={12} className="me-1"/> Open in New Tab
                      </a>
                    </div>
                ...
                {previewMedia.type === 'audio' && (
                  <div className="w-100 p-5 d-flex flex-column align-items-center justify-content-center" style={{ minHeight: '200px' }}>
                    <div className="mb-4 text-white opacity-75">
                       <PlaySquare size={48} />
                    </div>
                    <audio src={previewMedia.url} controls autoPlay className="w-75">
