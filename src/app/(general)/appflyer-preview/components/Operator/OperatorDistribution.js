...
const OperatorDistribution = ({ operatorData = [] }) => {
  const chartRef = useRef(null);
  const chartInstance = useRef(null);

  const processedData = useMemo(() => {
    const totalImpressions = operatorData.reduce((acc, item) => acc + Number(item.Impressions || item.impressions || 0), 0);
    
    // Vibrant Palette
    const palette = ["#3B82F6", "#6366F1", "#8B5CF6", "#EC4899", "#F43F5E", "#10B981", "#F59E0B", "#34D399"];

    return operatorData.map((item, idx) => {
