...
const UrlPerformance = ({ urlData = [] }) => {
  const chartRef = useRef(null);
  const chartInstance = useRef(null);

  useEffect(() => {
    if (chartRef.current && urlData.length > 0) {
      ...
      const labels = urlData.map(item => item.name || item.url || item.domain || "");
      const impressionsData = urlData.map(item => Number(item.Impressions || item.impressions || 0));
      const ctrData = urlData.map(item => {
