...
const UrlDistribution = ({ urlData = [] }) => {
  const chartRef = useRef(null);
  const chartInstance = useRef(null);

  const processedData = useMemo(() => {
    const totalImpressions = urlData.reduce((acc, item) => acc + Number(item.Impressions || item.impressions || 0), 0);
    
    // Vibrant Palette
    const palette = ["#3B82F6", "#6366F1", "#8B5CF6", "#EC4899", "#F43F5E", "#10B981", "#F59E0B", "#34D399"];

    return urlData.map((item, idx) => {
      ...
      return {
        label: item.name || item.url || item.domain || "",
        value: imp,
    ...
    <div className="card border-0 shadow-sm mb-4">
      <div className="card-header bg-white border-0 pt-4 px-4 pb-3">
        <div className="d-flex justify-content-between align-items-center w-100">
            <h5 className="mb-0 fw-bold text-dark" style={{ fontSize: '1.2rem' }}>URLs/Domains Distribution</h5>
        </div>
      </div>
...
export default UrlDistribution;
