...
const OperatorPerformance = ({ operatorData = [] }) => {
  const chartRef = useRef(null);
  const chartInstance = useRef(null);

  useEffect(() => {
    if (chartRef.current && operatorData.length > 0) {
      ...
      const labels = operatorData.map(item => item.name || item.operator || "");
      const impressionsData = operatorData.map(item => Number(item.Impressions || item.impressions || 0));
      const ctrData = operatorData.map(item => {
