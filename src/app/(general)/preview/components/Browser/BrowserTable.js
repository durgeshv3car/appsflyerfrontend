...
const BrowserTable = ({ browserData = [], currencySymbol = "$", campaignPermissions = [] }) => {
  ...
  const totalPages = Math.ceil((browserData?.length || 0) / rowsPerPage);
  const startIndex = (currentPage - 1) * rowsPerPage;
  const paginatedData = browserData?.slice(startIndex, startIndex + rowsPerPage) || [];

  const totals = browserData?.reduce((acc, row) => {
