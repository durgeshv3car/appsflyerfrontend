...
const BrowserTable = ({ 
  ...
  useEffect(() => {
    if (filteredColumnsByPermission.length > 0) {
      ...
      const stored = localStorage.getItem("visible_columns_browser");
      if (stored) {
        try {
          ...
          console.error("Failed to parse stored columns for Browser table", e);
        }
      }
  ...
  }, [browserData, campaignPricing, globalEffectiveMetrics, campaignType, appsflyerCampaignType, globalTotals, appsflyerDataLength, appsflyerData]);
  ...
  const handleApplyColumns = () => {
    ...
    localStorage.setItem("visible_columns_browser", JSON.stringify(tempVisibleColumns));
    setShow(false);
  };
    ...
    <div className="card border-0 shadow-sm mb-4">
      <div className="card-header bg-white border-0 pt-4 px-4 pb-3">
        <div className="d-flex justify-content-between align-items-center w-100">
            <h5 className="mb-0 fw-bold text-dark" style={{ fontSize: '1.2rem' }}>Browsers</h5>
          ...
          <div data-html2canvas-ignore="true" data-print-hide className="d-flex justify-content-end align-items-center gap-4 py-3 px-4 text-muted border-top bg-light-subtle" style={{ borderBottomLeftRadius: '12px', borderBottomRightRadius: '12px' }}>
              ...
              <div className="d-flex align-items-center gap-3">
                <span style={{ fontSize: '13px', fontWeight: '500', minWidth: '80px', textAlign: 'center' }}>
                  {startIndex + 1}-{Math.min(startIndex + rowsPerPage, browserData.length)} of {browserData.length}
                </span>
