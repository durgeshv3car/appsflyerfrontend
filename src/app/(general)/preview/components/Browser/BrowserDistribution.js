...
const BrowserDistribution = ({ browserData = [] }) => {
  const chartRef = useRef(null);
  const chartInstance = useRef(null);

  const processedData = useMemo(() => {
    const totalImpressions = browserData.reduce((acc, item) => acc + Number(item.Impressions || item.impressions || 0), 0);
