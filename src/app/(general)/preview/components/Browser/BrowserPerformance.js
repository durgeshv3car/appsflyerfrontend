...
const BrowserPerformance = ({ browserData = [] }) => {
  const chartRef = useRef(null);
  const chartInstance = useRef(null);

  useEffect(() => {
    if (chartRef.current && browserData.length > 0) {
      ...
      const labels = browserData.map(item => item.name || item.browser || "");
      const impressionsData = browserData.map(item => Number(item.Impressions || item.impressions || 0));
      const ctrData = browserData.map(item => {
