...
const OsTable = ({ osData = [], currencySymbol = "$", campaignPermissions = [] }) => {
  ...
  const totalPages = Math.ceil((osData?.length || 0) / rowsPerPage);
  const startIndex = (currentPage - 1) * rowsPerPage;
  const paginatedData = osData?.slice(startIndex, startIndex + rowsPerPage) || [];

  const totals = osData?.reduce((acc, row) => {
