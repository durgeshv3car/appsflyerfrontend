...
const OsTable = ({ 
  ...
  useEffect(() => {
    if (filteredColumnsByPermission.length > 0) {
      ...
      const stored = localStorage.getItem("visible_columns_os");
      if (stored) {
        try {
          ...
          console.error("Failed to parse stored columns for OS table", e);
        }
      }
  ...
  }, [osData, campaignPricing, globalEffectiveMetrics, campaignType, appsflyerCampaignType, globalTotals, appsflyerDataLength, appsflyerData]);
  ...
  const handleApplyColumns = () => {
    ...
    localStorage.setItem("visible_columns_os", JSON.stringify(tempVisibleColumns));
    setShow(false);
  };
