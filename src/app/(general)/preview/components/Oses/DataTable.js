...
const PerformanceTable = ({ tableData = [] }) => {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem("visible_columns_oses");
    if (stored) {
      try {
        ...
        console.error("Failed to parse stored columns for Oses table", e);
      }
    }
  ...
  const handleApplyColumns = () => {
    ...
    localStorage.setItem("visible_columns_oses", JSON.stringify(tempVisibleColumns));
    setShow(false);
  };
