...
const Menus = () => {
    ...
    const hasDropdown = Array.isArray(dropdownMenu) && dropdownMenu.length > 0;

    return (
        <li
            key={id}
            onClick={(e) => hasDropdown && handleMainMenu(e, name.split(' ')[0])}
            className={`nxl-item ${hasDropdown ? "nxl-hasmenu" : ""} ${activeParent === name.split(' ')[0] ? "active nxl-trigger" : ""}`}
        >
