...
const EstimateStatisticsTwo = ({ initialData }) => {
    // Use initialData if provided, otherwise fall back to static data
    const data = initialData?.length > 0 ? initialData : reportSalesStatisticsData;

    console.log("EstimateStatisticsTwo data:", data);


    return (
        <>
            {data.map(({ id, last_month_sales, revenue, running_month_sales, Sessions, icon }) => {
                                ...
                                <div className="hstack justify-content-between">
                                    <div>
                                        ...
                                        <h4 className="fw-bolder mb-3">
                                            $<span className="counter">{running_month_sales}</span> USD
                                        </h4>
                                        <p className="fs-12 text-muted mb-0">
                                            vs last month: <span className="fw-semibold text-dark">${last_month_sales} USD</span>
                                        </p>
                                    </div>

                                    <Link href="#" className={`badge ${badgeClass}`}>
                                        {symbol} {revenue}%
                                    </Link>
                                </div>
                            </div>
